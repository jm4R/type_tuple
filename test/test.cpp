...
struct test_fixture
{
    ...
    template < typename T > void test_explicit_type(T testValue)
    {
        ...
        static_assert(!std::is_convertible< T, velocity >::value,
            ...
            "underlying type");
        static_assert(!std::is_convertible< velocity, T >::value,
            "explicit_type shouldn't be explicitly convertible to underlying "
            "type");
    ...
    void test_type_tuple()
    {
        ...
        {
            db_options db{ auto_commit{ true }, use_cache{ true },
                cache_size{ 500 } };
            assert(true == db.get< use_cache >());
            assert(true == db.get< auto_commit >());
            assert(500 == db.get< cache_size >());
        ...
        {
            db_options db_tmp{ auto_commit{ true }, use_cache{ true },
                cache_size{ 500 } };
    ...
    void test_explicit_get_set()
    {
        ...
        using object
            = mj::explicit_type< std::shared_ptr< int >, class object_tag >;
        using movable_only
            = mj::explicit_type< std::unique_ptr< int >, class object_tag >;
        ...
        t.get< movable_only >()
            = movable_only{ std::unique_ptr< int >(new int{ 31 }) };
    ...
    void test_size()
    {
        ...
        static_assert(
            sizeof(int) == sizeof(t02), "size of underlying type should equal");
        ...
        static_assert(sizeof(plain_data) == sizeof(tuple_data),
...
int main()
{
    ...
    test.test_explicit_type< std::shared_ptr< int > >(
